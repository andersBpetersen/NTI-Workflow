...
docker build -t tickjf/nti-workflow:0.6.4 .

docker push tickjf/nti-workflow:0.6.4
```

Valgfrit — push også `latest`:

```powershell
docker tag tickjf/nti-workflow:0.6.4 tickjf/nti-workflow:latest
...
Giver: `registry.example.com/nti/nti-workflow:0.6.4`
...
Erstat `0.6.4` med den nye version.
...
- Brug versions-tags (`0.6.4`) som primær reference i drift
