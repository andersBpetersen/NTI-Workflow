...
docker build -t tickjf/nti-workflow:0.6.5 .

docker push tickjf/nti-workflow:0.6.5
```

Valgfrit — push også `latest`:

```powershell
docker tag tickjf/nti-workflow:0.6.5 tickjf/nti-workflow:latest
...
Giver: `registry.example.com/nti/nti-workflow:0.6.5`
...
```powershell
docker pull tickjf/nti-workflow:0.6.5
docker run -d --restart unless-stopped -p 8000:8000 --name nti-workflow tickjf/nti-workflow:0.6.5
```
...
Erstat `0.6.5` med den nye version.
...
- Brug versions-tags (`0.6.5`) som primær reference i drift
