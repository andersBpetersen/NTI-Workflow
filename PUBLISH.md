...
docker push tickjf/nti-workflow:0.7.0
```

Valgfrit — push også `latest`:

```powershell
docker tag tickjf/nti-workflow:0.7.0 tickjf/nti-workflow:latest
...
Giver: `registry.example.com/nti/nti-workflow:0.7.0`
...
```powershell
docker pull tickjf/nti-workflow:0.7.0
docker run -d --restart unless-stopped -p 8000:8000 --name nti-workflow tickjf/nti-workflow:0.7.0
```
...
Erstat `0.7.0` med den nye version ved næste release.
...
- Brug versions-tags (`0.7.0`) som primær reference i drift
