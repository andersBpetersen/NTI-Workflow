...
docker build -t tickjf/nti-workflow:0.6.6 .

docker push tickjf/nti-workflow:0.6.6
```

Valgfrit — push også `latest`:

```powershell
docker tag tickjf/nti-workflow:0.6.6 tickjf/nti-workflow:latest
...
Giver: `registry.example.com/nti/nti-workflow:0.6.6`
...
Erstat `0.6.6` med den nye version.
...
- Brug versions-tags (`0.6.6`) som primær reference i drift
